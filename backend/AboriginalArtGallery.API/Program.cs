...
// Ensure local directories exist early so StaticFiles middleware detects them
...
var key = builder.Configuration["JwtSettings:Key"] ?? throw new InvalidOperationException("JWT settings key is missing.");
...
var enableSwagger = app.Configuration.GetValue<bool>("AppSettings:EnableSwagger");
if (enableSwagger)
{
    app.UseSwagger();
    app.UseSwaggerUI();
...
var enableHsts = app.Configuration.GetValue<bool>("AppSettings:EnableHsts");
if (enableHsts)
{
    app.UseHsts();
}
...
// ─── Startup Data Seeding & Local Image Downloader ──────────────────────────
using (var scope = app.Services.CreateScope())
{
    ...
    void EnsureArtist(string name, string tribe, string country, int birthYear, string bio)
    {
        if (!db.Artists.Any(a => a.Name == name))
            db.Artists.Add(new AboriginalArtGallery.API.Models.Artist
            { 
    ...
    if (db.Artifacts.Count() < 10)
    {
        ...
        var seed = new List<AboriginalArtGallery.API.Models.Artifact>
        {
